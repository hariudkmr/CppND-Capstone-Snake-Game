...
Game::Game(std::size_t grid_width, std::size_t grid_height)
    : snake(grid_width, grid_height), wall(grid_width, grid_height),
      engine(dev()),
      random_w(0,
               static_cast<int>(
                   grid_width -
                   1)), // if equal to width then food on borders wont be seen
...
void Game::Run(Controller const &controller, Renderer &renderer,
  ...
  while (running) {
    ...
    controller.HandleInput(running, snake, pauseRequest,
                           restartRequest); // Feat1
    if (pauseRequest != 1) {
      ...
      if (gState == GameState::kActive)
        ...
        if (restartRequest) {
          ...
          bUpdateScore =
              true; // updateScore flag also set to true to record for new game
          restartRequest =
              false; // reset the flag to monitor the reset request status
        }
      }
    }
    ...
    if (frame_end - title_timestamp >= 1000) {
      renderer.UpdateWindowTitle(score, frame_count, snake.speed, pauseRequest,
                                 (gState != GameState::kActive)); // Feat1
      frame_count = 0;
...
void Game::PlaceFood() {
  ...
  while (true) {
    x = random_w(engine);
    y = random_h(engine);
    // Check that the location is not occupied by a snake / wall object before
    // placing food.
    if (!snake.ObjCell(x, y)) {
      // added check to ensure that food is not where there is a wall or
      // obstacle
      if (!wall.ObjCell(x, y)) {
        food.x = x;
        food.y = y;
...
/*
        Game::timerThreadFunction is a function that will run in a separate
   thread from the main it is used to provide a feature where the snake will
   slow down for 10 seconds after the score crosses 10.

        The slowSpeed variable is used as the predicate for the condition
   variable which is notified when the score equals 10. The unique lock is used
   here to make use of in a condition variable and also to allow to unlock for a
   period when the thread goes to sleep (for 10 seconds). The lock is then again
   locked in -order to restore the speed to close to the original value
...
void Game::timerThreadFunction(bool &running) {
  ...
  while (running) {
    ...
    if (score == 10) {
      // Idea here is to start a timer when the score passes 10 (signalled
      // through a condition variable) and before the timer starts we will
      // update the snake speed in a mutex
      ...
      uLock.lock();
      snake.speed +=
          0.1f; // should increase speed only if old game still continuing
      slowSpeed = false;
    }
  }
}